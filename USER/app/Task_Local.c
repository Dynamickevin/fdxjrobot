...
{
    bool bGpioVal = bOpen ; //一般是高电平使能，电机电源低电平使能
   ...
   // { 
    ...
    else if(box_str_cmp( devName , "Charge" )== 0)
	{
        ...
        GpioSetBit(GPIO_CAM_24V_EN,bGpioVal);
		GpioSetBit(GPIO_CAM_12V_EN,bGpioVal);
    }
	...
	else  if(box_str_cmp( devName , "ALL" )== 0)				//处理所有的外设电源
	{
        ...
        GpioSetBit(GPIO_CAM_24V_EN,bGpioVal);					//主摄像头24V
		GpioSetBit(GPIO_CAM_12V_EN,bGpioVal);					//主摄像头12V
	}
    else
    {
        return false;
		...
		*/
        ...
        if( gBatAutoCtrl.bOnBridge && (gCmdParaVal_Ints[0]!=0) )
		{
            if( (!gRbtState.bCX_SwCheck1) && (!gRbtState.bCX_SwCheck2) )
			{
			 ...
			 //如果处于桥上，无电源输入，主电机正常运动
			if (gBatAutoCtrl.bOnBridge_needmove)
			{
				...
				gBatAutoCtrl.bOnBridge_needmove = false;
				//stprintf(ID_DEBUG,"3\r\n");
			 }
        ...
        if ( vid >= 0 )
		{
            if( gCmdParaInfos.items[vid].value_str_len < sizeof(gRbtState.RobotState) )
			{
				...
				{ 
                    ...
                    GpioSetBit(GPIO_CAM_24V_EN,0);
					GpioSetBit(GPIO_CAM_12V_EN,0);
                }
            }
        }

		//codeCfg=比例系数,目标桥编码器长度一半
        gCmdParaVal_Ints[0] = 300;
        gCmdParaVal_Ints[1] = 750;
    ...
    else
    {
		...
		else
		{
        	...
        	//if ( vid > 0 ) { gUserAllCfg.SlvMtCfg.press_ok_min = atoi( gCmdParaInfos.items[vid].value ); }
       		//if ( vid > 0 ) 
			gUserAllCfg.SlvMtCfg.press_ok_min = (u16)(((float)gCmdParaVal_Ints[0])/2.34375+0.5f); //单位为牛顿
        	//vid = cmd_para_get_id_by_name(&gCmdParaInfos,"pmax");
       		...
       		//if ( vid > 0 ) 
       		gUserAllCfg.SlvMtCfg.press_ok_max = (u16)(((float)gCmdParaVal_Ints[1])/2.34375+0.5f); //单位为牛顿
        	...
        	//if ( vid > 0 )
        	gUserAllCfg.SlvMtCfg.press_mst_stop_min = (u16)(((float)gCmdParaVal_Ints[2])/2.34375+0.5f); //单位为牛顿
        	...
        	//if ( vid > 0 ) 
         	gUserAllCfg.SlvMtCfg.press_mst_stop_max = (u16)(((float)gCmdParaVal_Ints[3])/2.34375+0.5f); //单位为牛顿
    	}
