...
void zt_motor_slave_driver_cfg_init(void)
{
    ...
    else
    {
        ...
        gSlvMtCfg.bat_charge_full = 252;			//机器人充满时的电压
...
void zt_motor_slave_driver_init(void)
{
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA | RCC_APB2Periph_GPIOB | RCC_APB2Periph_GPIOE | RCC_APB2Periph_GPIOB , ENABLE);
	...
	//从动轮码盘有关参数
	gSlaveMtAnaly.CodeScale = 300;
    gSlaveMtAnaly.curBridgeCodeHalf  = 750;
    ...
    if( (gRbtState.bCX_SwCheck1) || (gRbtState.bCX_SwCheck2) ) 		//当前处于桥上遮挡处
	{
		...
		{ 
			...
			else								//无电源输入，关闭充电
			{
				...
				//BAT_CHARGE_CLOSE();           //切换到常开触点
				//stprintf(ID_DEBUG,"motor speed=0, no charge\r\n");
				//stprintf(ID_DEBUG,"6\r\n");
