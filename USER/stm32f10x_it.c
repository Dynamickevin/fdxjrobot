...
void SysTick_Handler(void)
{
	...
	INT8U  err;
	DATA_CONTROL_BLOCK  *dp0,*dp1,*dp2,*dp8,*dp10,*dp11;
	...
	{
		counter = 0;
		dp10= OSMemGet(p_msgBuffer,&err);
		if(OS_NO_ERR == err)
		{
			((DATA_CONTROL_BLOCK *)dp10)->type = AM2320_MSG; 
			if(OS_NO_ERR != OSQPost(timerQueue,(void*)dp10)) 	 
			{
				OSMemPut(p_msgBuffer,dp10);
			}
		...
		dp11= OSMemGet(p_msgBuffer,&err);
		if(OS_NO_ERR == err)
		{
			((DATA_CONTROL_BLOCK *)dp11)->type = SYS_LED_MSG; 
			if(OS_NO_ERR != OSQPost(timerQueue,(void*)dp11)) 	 
			{
				OSMemPut(p_msgBuffer,dp11);
			}
